...
/**
 * Adds selected artifact sizes without counting an analyzed subtree twice.
 * Core performs the same normalization before execution; keeping the display
 * calculation aligned makes the footer accurate while a selection is being
 * assembled.
 */
export function selectedCandidateBytes(
  ...
  return selected.reduce((total, candidate, index) => {
    const covered = selected
      .slice(0, index)
      .some((ancestor) => pathContains(ancestor.path, candidate.path));
    return covered ? total : total + candidate.sizeBytes;
  }, 0);
}
